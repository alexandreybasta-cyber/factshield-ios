...
const VERDICT_CONFIG = {
  TRUE: { label: 'Verified True', color: '#10B981', bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200', borderClass: 'verdict-border-true' },
  SUBSTANTIALLY_TRUE: { label: 'Substantially True', color: '#14B8A6', bg: 'bg-teal-50', text: 'text-teal-700', border: 'border-teal-200', borderClass: 'verdict-border-sub-true' },
  MISLEADING: { label: 'Misleading', color: '#F59E0B', bg: 'bg-amber-50', text: 'text-amber-700', border: 'border-amber-200', borderClass: 'verdict-border-misleading' },
  FALSE: { label: 'False', color: '#EF4444', bg: 'bg-red-50', text: 'text-red-700', border: 'border-red-200', borderClass: 'verdict-border-false' },
  UNVERIFIABLE: { label: 'Unverifiable', color: '#9CA3AF', bg: 'bg-gray-50', text: 'text-gray-600', border: 'border-gray-200', borderClass: 'verdict-border-unverifiable' },
};

// ─── Helpers ─────────────────────────────────────────────────────────────────
function getVerdictColor(verdict) {
  return VERDICT_CONFIG[verdict]?.color || '#9CA3AF';
}

function renderVerdictBadge(verdict) {
  const cfg = VERDICT_CONFIG[verdict] || VERDICT_CONFIG.UNVERIFIABLE;
  return `<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${cfg.bg} ${cfg.text} ${cfg.border} border">${cfg.label}</span>`;
}
...
function renderSourceCard(source, idx) {
  ...
  const supportsBadge = source.supportsClaim
    ? '<span class="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 font-medium">Supports</span>'
    : '<span class="text-[10px] px-1.5 py-0.5 rounded-full bg-red-50 text-red-700 border border-red-200 font-medium">Refutes</span>';
  const credWidth = Math.round((source.credibility || 0.5) * 100);
  return `
    <div class="flex flex-col gap-1 py-2 border-b border-gray-100 last:border-0">
      <div class="flex items-center justify-between gap-2">
        <a href="${source.url || '#'}" target="_blank" rel="noopener" class="text-xs text-fs-accent font-medium hover:underline truncate flex-1">${domain}</a>
        ${supportsBadge}
      </div>
      <p class="text-[11px] text-fs-muted line-clamp-2">${source.title || ''}</p>
      <div class="flex items-center gap-1.5">
        <span class="text-[10px] text-gray-400">Credibility</span>
        <div class="flex-1 h-1 bg-gray-100 rounded-full max-w-[60px]">
          <div class="h-full rounded-full bg-fs-accent/60" style="width:${credWidth}%"></div>
        </div>
      </div>
    </div>`;
}
...
function showToast(message) {
  ...
  toast.id = 'toast';
  toast.className = 'fixed top-3 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs px-4 py-2 rounded-lg shadow-lg toast-enter z-50';
...
function renderHeader() {
  const statusDot = state.isActive
    ? '<span class="w-2 h-2 rounded-full bg-fs-accent animate-blink"></span>'
    : '<span class="w-2 h-2 rounded-full bg-gray-300"></span>';
  const statusText = state.isActive ? 'Monitoring' : 'Idle';
  return `
    <div class="header-accent h-1"></div>
    <header class="flex items-center justify-between px-4 py-3 border-b border-fs-border bg-white sticky top-0 z-40">
      <div class="flex items-center gap-2.5">
        <div class="w-8 h-8 bg-gradient-to-br from-orange-400 to-orange-600 rounded-xl flex items-center justify-center text-sm shadow-md shadow-orange-200">🛡️</div>
        <div>
          <h1 class="text-sm font-bold leading-none tracking-tight text-fs-text">FactShield</h1>
      ...
      <button onclick="openOptions()" class="p-2 rounded-lg hover:bg-gray-100 transition-colors" title="Open Settings">
        <svg class="w-[18px] h-[18px] text-fs-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
      </button>
    </header>`;
}

function renderControls() {
  const liveActive = state.mode === 'live' ? 'bg-fs-accent text-white shadow-sm' : 'text-fs-muted hover:text-fs-text';
  const manualActive = state.mode === 'manual' ? 'bg-fs-accent text-white shadow-sm' : 'text-fs-muted hover:text-fs-text';

  let controlContent = '';
  if (state.mode === 'live') {
    if (state.isActive) {
      controlContent = `
        <button onclick="stopFactCheck()" class="btn-press w-full py-2.5 px-4 bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 rounded-xl font-medium text-sm transition-all flex items-center justify-center gap-2">
      ...
      controlContent = `
        <button onclick="startFactCheck()" class="btn-press btn-orange-gradient w-full py-3 px-4 text-white rounded-xl font-semibold text-sm shadow-btn flex items-center justify-center gap-2 glow-active">
    ...
    controlContent = `
      <textarea id="manual-input" placeholder="Paste text to fact-check..." class="w-full h-20 bg-fs-surface border border-fs-border rounded-xl px-3 py-2.5 text-sm text-fs-text placeholder-gray-400 resize-none focus:outline-none focus:border-fs-accent focus:ring-2 focus:ring-orange-100 transition-all"></textarea>
      <button onclick="checkText()" class="btn-press btn-orange-gradient w-full py-2.5 px-4 text-white rounded-xl font-medium text-sm mt-2 flex items-center justify-center gap-2 shadow-btn">
  ...
  return `
    <section class="px-4 py-3 border-b border-fs-border bg-white">
      <div class="flex bg-fs-surface rounded-lg p-0.5 mb-3 border border-fs-border">
...
function renderPipeline() {
  ...
  return `
    <section class="px-4 py-3 border-b border-fs-border bg-orange-50/50 animate-fade-in">
      <div class="flex items-center gap-2 mb-2">
        <div class="w-4 h-4 border-2 border-fs-accent border-t-transparent rounded-full animate-spin-slow"></div>
        <span class="text-xs text-fs-muted font-medium">${escapeHtml(msg)}</span>
      </div>
...
function renderVerdictCards() {
  ...
  return `
    <section class="flex-1 overflow-y-auto custom-scrollbar px-4 py-3 space-y-3 bg-fs-surface">
...
function renderSingleCard(claim, idx) {
  ...
  if (!hasVerdict) {
    return `
      <div class="verdict-card verdict-border-pending bg-white border border-fs-border rounded-xl p-3.5 shadow-card animate-slide-in">
        <div class="flex items-start gap-2.5">
          ...
          <div class="flex-1">
            ...
            <p class="text-[11px] text-fs-accent mt-1.5 font-medium">Verifying...</p>
          </div>
        </div>
      </div>`;
  }
  ...
  return `
    <div class="verdict-card ${cfg.borderClass} bg-white border border-fs-border rounded-xl p-3.5 shadow-card animate-slide-in">
      <div class="flex items-start justify-between gap-2 mb-2">
        ${renderVerdictBadge(claim.verdict)}
        <span class="text-[10px] text-gray-400 shrink-0">${formatTimestamp(claim.timestamp)}</span>
      </div>
      ...
      <div class="mt-2.5">
        ...
        ${reasoning.length > 100 ? `<button onclick="toggleCard('${id}')" class="text-[10px] text-fs-accent font-medium hover:underline mt-1">${isExpanded ? 'Show less' : 'Show more'}</button>` : ''}
      </div>

      <!-- Sources -->
      ${sources.length > 0 ? `
        <div class="mt-2.5 pt-2.5 border-t border-gray-100">
          <button onclick="toggleSources('${id}')" class="flex items-center gap-1 text-[11px] text-fs-muted hover:text-fs-text transition-colors font-medium">
      ...
      <div class="flex justify-end mt-2">
        <button onclick="copyVerdict(state.claims[${idx}])" class="text-[10px] text-gray-400 hover:text-fs-accent flex items-center gap-1 transition-colors">
...
function renderEmptyState() {
  ...
  return `
    <section class="flex-1 flex flex-col items-center justify-center px-6 py-12 text-center bg-fs-surface">
      <div class="w-16 h-16 bg-orange-50 rounded-2xl flex items-center justify-center mb-4 border border-orange-100">
        <span class="text-3xl">🔍</span>
      </div>
      <h3 class="text-sm font-semibold text-fs-text mb-1">Ready to fact-check</h3>
      <p class="text-xs text-fs-muted leading-relaxed mb-4">Start monitoring a page or paste text to verify claims in real-time.</p>
      <div class="bg-white rounded-lg px-4 py-3 border border-fs-border shadow-card">
        <p class="text-[11px] text-fs-muted leading-relaxed">💡 Works best on YouTube, news articles, and podcasts</p>
      </div>
    </section>`;
}
  ...
  if (!state.hasApiKey) {
    return `
      <section class="px-4 py-3">
        <div class="bg-orange-50 border border-orange-200 rounded-xl p-3.5">
          <div class="flex items-start gap-2.5">
            <span class="text-base">⚙️</span>
            <div>
              <p class="text-xs text-orange-800 font-semibold">API Key Required</p>
              <p class="text-[11px] text-orange-600 mt-0.5">Please configure your API keys in Settings to start fact-checking.</p>
              <button onclick="openOptions()" class="text-[11px] text-fs-accent font-semibold hover:underline mt-2 inline-block">Open Settings →</button>
            </div>
          </div>
        </div>
      </section>`;
        ...
        <div class="bg-red-50 border border-red-200 rounded-xl p-3.5">
          <div class="flex items-start gap-2.5">
            ...
            <div>
              ...
              <p class="text-[11px] text-red-600 mt-0.5">${escapeHtml(state.error)}</p>
            </div>
          </div>
        </div>
      </section>`;
...
function renderHistory() {
  ...
  return `
    <section class="border-t border-fs-border bg-white">
      <button onclick="toggleHistory()" class="w-full px-4 py-2.5 flex items-center justify-between hover:bg-gray-50 transition-colors">
        ...
        ${state.history.slice(0, 10).map(h => `
          <div class="py-2 border-b border-gray-100 last:border-0">
            <div class="flex items-center justify-between">
              ...
              <span class="text-[10px] text-gray-400">${formatTimestamp(h.date)}</span>
            </div>
