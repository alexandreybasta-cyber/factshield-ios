...
## Introduction
This document provides comprehensive documentation for the APIClient base class that powers all API communications in the FactShield iOS application. It explains the generic request methods, JSON request handling, response validation mechanisms, error handling system, retry logic with exponential backoff, timeout configurations, URLSession configuration, logging integration, and performance monitoring capabilities. It also includes practical examples for extending the base client for new API integrations, managing custom headers and authentication tokens, and outlines best practices for thread safety, memory management, debugging, and troubleshooting.

## Project Structure
The networking layer is organized around a central APIClient actor that encapsulates HTTP communication, error handling, and retry logic. Specific providers (e.g., QwenAPI, TavilySearchProvider, GoogleFactCheckProvider) build on top of APIClient to implement provider-specific request bodies, headers, and response parsing.
...
Both methods implement retry logic with exponential backoff and validate HTTP responses. Providers such as QwenAPI and SearchAPI construct request bodies, set headers (including Authorization), and call APIClient methods.
...
- Builds request body from arrays of message dictionaries.
...
- Defines a protocol for pluggable search providers.
- Implements TavilySearchProvider and GoogleFactCheckProvider.
- Both providers:
  - Retrieve API keys from environment or UserDefaults.
  - Construct request bodies or query strings.
  - Call APIClient.requestJSON to parse provider-specific JSON.
  - Parse results into a unified SearchResult model.
...
Best practices:
- Store tokens securely (e.g., Keychain) in production.
- Prefer environment variables for local development.
- Avoid hardcoding tokens in source.
...
2. Load credentials from secure storage or environment.
...
- Memory management:
  ...
  - Ensure request bodies are constructed efficiently and avoid retaining large closures.
...
- Invalid URL:
  ...
  - Resolution: Verify base URLs and endpoint paths.
- Invalid response:
...
## Conclusion
The APIClient actor provides a robust foundation for API communications with built-in retry logic, structured error handling, and logging. By leveraging APIClient.shared, providers can implement consistent patterns for request construction, authentication, and response parsing while maintaining thread safety and performance. Following the best practices outlined here ensures reliable and maintainable API integrations across the FactShield application.
